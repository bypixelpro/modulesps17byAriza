...
class getProductByCatId {
    ...
    public function __construct($_catId, $_numProducts = 1) {
    ...
    private function getLastProductByCatId($_catId, $_numProducts) {
        ...
        if(is_numeric($_catId) && $_catId > 0) {
            ...
            foreach($getProducts as $key => $val) {
                ...
                $getProducts[$key]['price'] = Tools::convertPrice($val['price'], Context::getContext()->currency, true, Context::getContext()) . ' ' . $currency->sign;
                // Get Image Cover Product
                $cover =  $singleProduct->getCover($val['id_product']);
                if (sizeof($cover["id_image"]) && $cover["id_image"] > 0) {                    
                    $image = new Image($cover['id_image']);
                    $getProducts[$key]['coverProduct']['legend'] = isset($image->lengend[1]) && !empty($image->lengend[1]) ? $image->legend[1] : $val['name'];
                    $getProducts[$key]['coverProduct']['url'] = _PS_BASE_URL_._THEME_PROD_DIR_.$image->getExistingImgPath().'.'.$image->image_format;                 
                }
            }  
        }
        return $getProducts;
